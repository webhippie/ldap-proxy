DIST := dist
IMPORT := github.com/webhippie/ldap-proxy

ifeq ($(OS), Windows_NT)
	EXECUTABLE := ldap-proxy.exe
else
	EXECUTABLE := ldap-proxy
endif
...
.PHONY: install
install: $(SOURCES)
	go install -v -tags '$(TAGS)' -ldflags '$(LDFLAGS)' $(IMPORT)/cmd/ldap-proxy

.PHONY: build
build: $(EXECUTABLE)

$(EXECUTABLE): $(SOURCES)
	go build -v -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $@ $(IMPORT)/cmd/ldap-proxy
...
release-build:
	...
	xgo -dest $(DIST)/binaries -tags '$(TAGS)' -ldflags '-s -w $(LDFLAGS)' -targets '$(TARGETS)' -out $(EXECUTABLE)-$(VERSION) $(IMPORT)/cmd/ldap-proxy
ifeq ($(CI),drone)
